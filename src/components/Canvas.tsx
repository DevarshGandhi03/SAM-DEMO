...
interface CanvasProps {
  konvaRef: React.RefObject<Konva.Stage> | any;
  ...
  containerRef: RefObject<HTMLDivElement> | any;
  hasClicked: boolean;
...
const Canvas = ({
  ...
  const animateAllSvg = () => {
    ...
    const animateAllSvg:any =
      konvaRef.current.findOne(".animateAllSvg");
    animateAllSvg.find("Circle").forEach((el:any) => el.destroy());
    animateAllSvg.find("Path").forEach((el:any) => el.destroy());
  ...
  useEffect(() => {
    ...
    if (el) {
      ...
      el.scrollTop = padding + maxScrollTop / 2;
    }
  }, [scalingStyle]);
    ...
    <>
      ...
      <div
        className={`absolute w-full h-full overflow-auto Canvas-wrapper md:overflow-visible md:w-auto md:h-auto absolute-center ${
          !isStandalone ? "pt-36 pb-14 md:p-0" : ""
        }`}
        ref={scrollRef}
      >
        <div
